# collect the sample dataset using carla
...
def process_car_mask(mask, min_pixels=700):
    ...
    if stats[largest_label, 4] < min_pixels:
        return False, mask
    ...
    with tqdm(total=total_samples, desc="Collecting Samples", unit="img") as pbar:
        ...
        for v_idx, vehicle_id in enumerate(vehicle_ids):
            ...
            try:
                ...
                skipped = (
                    locations_per_car * len(distances) * len(pitches) * shots_per_orbit
                )
                pbar.update(skipped)
                continue

            # iterate over spawn points / locations
            chosen_spawns = random.sample(
                range(n_spawn_points), min(locations_per_car, n_spawn_points)
            )
